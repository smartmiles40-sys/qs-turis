...
export default function WaConversation({ leadId, leadName, phone, initialText }: Props) {
  ...
  // MODO APARELHO (SDR, desde 03/09): esta tela vira HISTÓRICO. Continua sendo
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      {/* ── ESCREVER ────────────────────────────────────────────────────────
          No MODO APARELHO o campo de escrever dá lugar a um atalho: o envio
          não sai daqui, e um campo que aceita texto e depois recusa o envio é
          pior que campo nenhum — o SDR digita a mensagem inteira pra descobrir
          no fim que ela não vai. O histórico acima continua completo. */}
      {modoApp ? (
        <div className="shrink-0 border-t px-3 py-3" style={{ borderColor: "var(--line)", background: "var(--card)" }}>
          <div className="flex items-center gap-3">
            <div className="flex-1 min-w-0">
              <p className="text-[12.5px] font-bold" style={{ color: "var(--ink)" }}>
                Esta tela é o histórico
              </p>
            ...
            <button
              ...
              disabled={!phone}
              className="shrink-0 inline-flex items-center gap-1.5 px-3.5 h-10 rounded-xl text-[13px] font-bold text-white disabled:opacity-50"
              style={{ background: "#25D366" }}
              title={phone ? "Abrir esta conversa no WhatsApp do seu celular" : "Lead sem telefone cadastrado"}
            >
