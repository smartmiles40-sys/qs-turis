// src/components/sdr/agenda/AgendaClosers.tsx
// -----------------------------------------------------------------------------
...
/** Reunião ainda "viva" — ocupa horário e pode virar desfecho. */
const VIVAS: MeetingStatus[] = ["agendada", "confirmada"];
...
function distribuirEmFaixas(ms: Meeting[]): ComFaixa[] {
  ...
  for (const m of ordenados) {
    ...
    atual.push(m);
    fimDoBloco = Math.max(fimDoBloco, fimDe(m).getTime());
  }
  ...
  for (const bloco of blocos) {
    ...
    for (const m of bloco) {
      ...
      if (idx === -1) {
        fimDaFaixa.push(fimDe(m).getTime());
        idx = fimDaFaixa.length - 1;
      } else {
        fimDaFaixa[idx] = fimDe(m).getTime();
      }
...
export default function AgendaClosers({ onOpenLead, demo }: AgendaClosersProps) {
  ...
  const contagens = useMemo(() => {
    ...
    for (const m of doDia) {
      ...
      if (pendente(m, agora)) c.pendentes += 1;
    }
    return c;
  }, [doDia, agora]);
  ...
  const visiveis = useMemo(() => {
    ...
    if (filtro === "pendentes") return doDia.filter((m) => pendente(m, agora));
  ...
  const porColuna = useMemo(() => {
    ...
    for (const col of colunas) {
      mapa[col.key] = distribuirEmFaixas(visiveis.filter((m) => chaveDaColuna(m) === col.key));
    }
    return mapa;
  }, [colunas, visiveis]);
  ...
  const [horaInicio, horaFim] = useMemo(() => {
    ...
    for (const m of doDia) {
      ...
      fim = Math.max(fim, Math.ceil(minutosDoDia(fimDe(m)) / 60));
    }
    if (ehHoje) {
  ...
  return (
    ...
    >
      ...
      <div ref={gradeRef} className="relative flex-1 overflow-auto">
        ...
        <div style={{ minWidth: LARGURA_EIXO + colunas.length * LARGURA_MIN_COLUNA }}>
          <div ref={cabecalhoRef} className="sticky top-0 z-40 flex border-b border-gray-200 bg-gray-50">
            ...
            {colunas.map((col) => {
              const dele = doDia.filter((m) => chaveDaColuna(m) === col.key);
              const pend = dele.filter((m) => pendente(m, agora)).length;
              return (
                <div
                  key={col.key}
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="truncate text-xs text-gray-500">
                      ...
                      {pend > 0 && <span className="ml-1 font-semibold text-red-600">· {pend} pendente(s)</span>}
                    </div>
                  </div>
                </div>
              );
            ...
            {colunas.map((col) => (
              ...
              >
                ...
                {(porColuna[col.key] ?? []).map(({ m, faixa, totalFaixas }) => {
                  ...
                  const atrasada = pendente(m, agora);
      ...
      {selecionada && (
        <PainelReuniao
          reuniao={selecionada}
          coluna={colunas.find((c) => c.key === chaveDaColuna(selecionada))}
          agora={agora}
...
function PainelReuniao({ reuniao, coluna, agora, onFechar, onStatus, onSal, onRemarcar, onAbrirLead }: PainelProps) {
  ...
  const fim = fimDe(reuniao);
  const atrasada = pendente(reuniao, agora);
